...
<form method="POST" action="{{ route('admin.rooms.update', $room->uuid) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input id="name" name="name" type="text"
                               ...
                               placeholder="cth. Kamar Deluxe" required
                               class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-yellow-400 focus:border-yellow-400 outline-none transition-all {{ $errors->has('name') ? 'border-red-500 focus:ring-red-400' : '' }}"
                               maxlength="255">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input id="name_en" name="name_en" type="text"
                               value="{{ old('name_en', $room->name_en) }}"
                               placeholder="e.g. Deluxe Room"
                               class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-yellow-400 focus:border-yellow-400 outline-none transition-all {{ $errors->has('name_en') ? 'border-red-500' : '' }}"
                               maxlength="255">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select id="status" name="status" required
                            ...
                            <option value="available"   {{ old('status', $room->status) === 'available'   ? 'selected' : '' }}>Tersedia (Siap disewa)</option>
                            <option value="unavailable" {{ old('status', $room->status) === 'unavailable' ? 'selected' : '' }}>Tidak Tersedia (Maintenance/Penuh)</option>
                        </select>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="description" name="description" rows="5" maxlength="5000"
                                  placeholder="Tulis informasi keunggulan kamar ini..."
                                  class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-yellow-400 focus:border-yellow-400 outline-none transition-all resize-y {{ $errors->has('description') ? 'border-red-500' : '' }}">{{ old('description', $room->description) }}</textarea>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="description_en" name="description_en" rows="5" maxlength="5000"
                                  placeholder="Write the room's strengths and features in English..."
                                  class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-yellow-400 focus:border-yellow-400 outline-none transition-all resize-y {{ $errors->has('description_en') ? 'border-red-500' : '' }}">{{ old('description_en', $room->description_en) }}</textarea>
            ...
            <div class="bg-white p-6 md:p-8 rounded-2xl border border-slate-200 shadow-sm">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 border-b border-slate-100 pb-4">
                    ...
                    <button type="button" onclick="addFacility()"
                            class="inline-flex items-center justify-center gap-2 px-4 py-2 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 text-sm font-semibold rounded-xl transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="lg:col-span-4 flex flex-col gap-8">
            ...
            <div class="bg-white p-6 md:p-8 rounded-2xl border border-slate-200 shadow-sm">
                ...
                @if($room->photos->count() > 0)
                    <p class="text-xs text-slate-500 mb-3">Foto saat ini — klik tanda silang untuk menandai dihapus.</p>
                    <div class="photo-preview-grid grid grid-cols-3 gap-3 mb-5 max-h-48 overflow-y-auto pr-1" id="existingGrid">
                        @foreach($room->photos as $photo)
                            <div class="relative aspect-square rounded-xl overflow-hidden border-2 transition-colors {{ $photo->is_cover ? 'border-yellow-400' : 'border-slate-200' }}"
                                 id="ep_{{ $photo->id }}">
                                <img src="{{ asset($photo->path) }}" class="w-full h-full object-cover" alt="">
                                @if($photo->is_cover)
                                    <span class="absolute bottom-1.5 left-1.5 bg-yellow-400 text-yellow-900 text-[0.6rem] font-bold px-2 py-0.5 rounded-full z-10">COVER</span>
                                @endif
                                <button type="button"
                                        onclick="toggleDeletePhoto({{ $photo->id }})"
                                        title="Tandai hapus"
                                        class="absolute top-1.5 right-1.5 w-6 h-6 rounded-full bg-slate-900/60 text-white flex items-center justify-center hover:bg-red-500 transition-colors z-10">
                ...
                {{-- Upload foto baru --}}
                <div class="photo-zone border-2 border-dashed border-slate-300 rounded-2xl p-6 text-center cursor-pointer transition-all hover:border-yellow-400 hover:bg-yellow-50"
                     id="photoZone"
                    ...
                    <p class="text-sm font-semibold text-slate-700">Tambah foto baru</p>
            ...
            <div class="bg-white p-6 md:p-8 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <div class="space-y-5">
                    ...
                    <div id="discountFields"
                         class="{{ old('discount_type', $room->discount_type) === 'none' ? 'hidden' : '' }} space-y-5 border-t border-slate-100 pt-5">
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1.5" for="discount_value">
                                Besaran Diskon
                                <span id="discountUnit" class="text-slate-400 font-normal">
                            ...
                            <input id="discount_value" name="discount_value" type="number"
                                   ...
                                   min="0" step="any" placeholder="cth. 15"
    ...
    <div class="mt-8 pt-6 border-t border-slate-200 flex flex-col-reverse sm:flex-row justify-end gap-3 sticky bottom-0 bg-slate-50/90 backdrop-blur-sm pb-6 z-10">
        <a href="{{ route('admin.rooms.index') }}"
           class="inline-flex justify-center items-center px-6 py-3 bg-white border border-slate-300 text-slate-700 text-sm font-semibold rounded-xl hover:bg-slate-50 transition-colors sm:w-auto w-full">
            Batal
        </a>
        <button type="submit"
                class="inline-flex justify-center items-center gap-2 px-8 py-3 bg-yellow-500 hover:bg-yellow-600 text-yellow-950 text-sm font-bold rounded-xl shadow-sm hover:shadow transition-all sm:w-auto w-full">
...
/* ── Toggle mark-delete untuk foto existing ── */
function toggleDeletePhoto(id) {
    ...
    if (deletedPhotos.has(id)) {
        ...
        overlay.className = 'absolute inset-0 bg-red-500/60 flex items-center justify-center text-xs font-bold text-white z-20 backdrop-blur-[1px] deleted-overlay rounded-lg';
...
function renderPreviews() {
    ...
    selectedFiles.forEach((file, i) => {
        ...
        reader.onload = function(e) {
            ...
            item.className = 'relative aspect-square rounded-xl overflow-hidden border-2 border-slate-200';
            item.innerHTML = `
                <img src="${e.target.result}" alt="Preview" class="w-full h-full object-cover">
                <button type="button" onclick="removePhoto(${i})" title="Hapus"
                    class="absolute top-1.5 right-1.5 w-6 h-6 rounded-full bg-slate-900/60 text-white flex items-center justify-center hover:bg-red-500 transition-colors backdrop-blur-sm">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12"/>
                    </svg>
                </button>
...
// Intercept form submit untuk inject file baru
